...
- Hard-rule validator винесений у серверний валідатор і використовується як у тестах, так і перед збереженням нових чернеток.
- Локальний integration harness для довгих scale-тестів автогенерації не публікується в репозиторій; якщо він є у робочій копії, для запуску матриці діапазонів або всього курсу встановіть `RUN_LONG_AUTOGEN_SCALE=1`.
