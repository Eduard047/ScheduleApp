...
За замовчуванням `AllowedHosts` має значення `*` і приймає будь-який непорожній HTTP-заголовок `Host`, що зручно для приватного екземпляра, доступного за IP-адресою або різними DNS-іменами. За потреби обмежте розгортання точними іменами через змінну середовища, наприклад `AllowedHosts=schedule.example.edu;admin.schedule.example.edu`; значення `+` та шаблони на кшталт `*.example.edu` відхиляються. Адресу прослуховування Kestrel задавайте окремо через `ASPNETCORE_URLS`.
