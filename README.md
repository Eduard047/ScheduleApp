...
- Фінальне дозаповнення виконує детермінований residual-пошук: спочатку максимізується кількість заповнених слотів, а потім мінімізується вартість призначень. План застосовується атомарно й повністю відкочується, якщо будь-яке призначення не проходить hard rules. Один відтворюваний бюджет вузлів діє для групи та дати в межах усього запуску; аварійний тайм-аут лише захищає процес від патологічного зависання.
- Причини незаповнених слотів мають структуровані `reasonCode`, `constraintCode`, ознаку вичерпання пошуку та діагностику. Текстове поле причини збережене для сумісності зі старими журналами.
- Якість алгоритму контролює зафіксована синтетична матриця: короткі профілі та 18-тижневий refill-сценарій на 324 заняття перевіряють KPI, збереження locked-рядків, відсутність `search-limit`, hard-rule порушення, стабільний fingerprint і повторюються тричі в щоденному CI-запуску.
