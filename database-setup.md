...
Актуальне оновлення схеми також додає `ScheduleItems.BatchKey` для атомарної роботи з одним логічним заняттям, маркери `Revision` для захисту розкладу й чернеток від застарілих паралельних змін та унікальні індекси конфігурації для кодів типів/модулів, слотів часу, календарних винятків, обіду й ліміту першого слота. Міграція заповнює ревізії наявних рядків, а наступна міграція задає для нових рядків MySQL-default `(UUID())`, щоб пряме або застаріле створення запису не залишало порожню версію. Перед оновленням робочої бази зробіть резервну копію та перевірте процедуру у staging-середовищі.

Перед застосуванням revision-міграцій зупиніть старий екземпляр сервера та дочекайтеся завершення його активних запитів. Default захищає нові вставки, але старий код не оновлює `Revision` під час зміни наявного рядка, тому одночасний запуск старої та нової версій не підтримується.
